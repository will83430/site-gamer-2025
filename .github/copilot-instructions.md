...
- Stack: Node 18+, Express server + PostgreSQL (`gamer_2025`), static frontend under `frontend/public`, generated fiches under `fiches/`; gzip via compression middleware and permissive CORS in [server.js](server.js).
- Static serving order matters: `/assets` → `/frontend/public` → `/fiches` → project root (see [server.js](server.js)). Keep it when adding middleware.
- DB access is hard-coded to user `postgres`, db `gamer_2025`, password `Wilfried!1985` across [server.js](server.js) and scripts in `scripts/`. Align credentials if you change them.
...
- API products: `/api/produits` GET with optional `?categorie=`; POST auto-assigns next `prod_*` id based on max substring; PUT expects all fields (not partial) and overwrites arrays; DELETE returns product name (see CRUD handlers in [server.js](server.js)). `image_url` is derived (defaults to `/assets/images/placeholder.png`).
...
- Tendances CRUD: `/api/tendances` uses the same category lookup pattern and expects `tags` array to be turned into `{...}` string for SQL (see [server.js](server.js)).
...
- Data seeding: bulk JSON inserts live in [backend/gestion_produits.sql](backend/gestion_produits.sql); package scripts mention `backend/schema.sql` but the repo mainly provides this populated dataset.
...
- Keep `slugToTitreAffiche()` at the end of [server.js](server.js) in mind: when `titre_affiche` is absent, POST generation will prettify the slug for display.
