...
// Détection automatique de l'IP pour mobile
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Configurer le bouton des tendances
    const btnTendances = document.getElementById('btn-tendances-categorie');
    if (btnTendances) {
        if (categorieActuelle) {
            btnTendances.href = `tendances-${categorieActuelle.toLowerCase().replace(/ /g, '-').replace(/_/g, '-')}.html`;
            btnTendances.innerHTML = `🔎 Voir les tendances ${categorieActuelle.charAt(0).toUpperCase() + categorieActuelle.slice(1).toLowerCase()}`;
...
function afficherProduitsCategorie(categorie) {
    ...
    console.log('🔍 Recherche cache pour:', categorieCache);
    ...
    if (cachedCategory && Array.isArray(cachedCategory) && cachedCategory.length > 0) {
        console.log('✅ CACHE HIT pour catégorie:', categorie, '- Produits:', cachedCategory.length);
    ...
    console.log('❌ CACHE MISS pour catégorie:', categorieCache);
    
    // CHARGEMENT NORMAL (première fois)
    const produitsFilters = tousLesProduits.filter(p => 
        p.categorie && p.categorie.toLowerCase() === categorieCache
    );
    
    console.log('💾 Sauvegarde en cache:', categorieCache, '- Produits:', produitsFilters.length);
    ...
    if (produit.lien.startsWith('http')) {
        ...
        return `
            <div class="fiche-produit" data-id="${produit.id}" style="position: relative; ${produit.top_du_mois ? 'border: 3px solid #ffd700; box-shadow: 0 0 20px rgba(255, 215, 0, 0.5);' : ''}">
                ${produit.top_du_mois ? '<span class="vedette-badge" style="background: linear-gradient(45deg, #ff6b6b, #ffd700); color: white; padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: bold; position: absolute; top: 10px; right: 10px; z-index: 10; animation: pulse 2s infinite; box-shadow: 0 2px 8px rgba(255, 107, 107, 0.3);">⭐ TOP</span>' : ''}
...
function setupComparisonCheckboxes() {
    ...
    newCheckboxes.forEach(checkbox => {
        checkbox.addEventListener('change', (e) => {
            ...
            console.log('🔄 Produits sélectionnés:', produitsSelectionnes.length);
        });
    });
}
...
async function comparerProduits() {
    ...
    try {
        ...
        // Créer les cartes de comparaison avec le MÊME STYLE que la grille principale
        ...
        ${produitsDetails.map(p => {
            ...
            return `
                <div class="fiche-comparaison" style="position: relative; ${p.top_du_mois ? 'border: 3px solid #ffd700; box-shadow: 0 0 20px rgba(255, 215, 0, 0.5);' : ''}">
                    ${p.top_du_mois ? '<span class="vedette-badge" style="background: linear-gradient(45deg, #ff6b6b, #ffd700); color: white; padding: 4px 8px; border-radius: 12px; font-size: 12px; font-weight: bold; position: absolute; top: 10px; right: 10px; z-index: 10; animation: pulse 2s infinite; box-shadow: 0 2px 8px rgba(255, 107, 107, 0.3);">⭐ TOP</span>' : ''}
                    ...
                    <p class="info"><strong>Prix:</strong> ${p.prix || 'Prix non communiqué'}</p>
                    ...
                    ${p.fonctionnalites_avancees && p.fonctionnalites_avancees.length > 0 ? `
                        <ul style="text-align: left; padding-left: 20px; margin: 10px 0;">
                            ${p.fonctionnalites_avancees.slice(0, 3).map(f => `<li style="font-size: 1.em;">✓ ${f}</li>`).join('')}
                        </ul>
                    ` : ''}
                </div>
            `;
...
`;
        ...
        // Attacher l'event listener avec setTimeout pour être sûr que le DOM est prêt
setTimeout(() => {
...
function fermerComparaison() {
    console.log('🔄 Fermeture de la comparaison...');
    ...
    // ✅ NOUVEAU : Réinitialiser les event listeners des checkboxes
    setupComparisonCheckboxes();
    
    console.log('✅ Comparaison fermée et checkboxes réinitialisées');
}
